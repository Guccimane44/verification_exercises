...
public class RunLengthTests {
    ...
    @Test
    void sumUnitTest(){
        List<Run<Integer>> input = List.of(new Run<>(1,4), new Run<>(2,3));
        Integer expected = 10;
        Assertions.assertEquals(expected, RunLength.sum(input));
    }
    ...
    void optimizedSum(@ForAll List<Integer> input) {
        ...
        Assertions.assertEquals(RunLength.sum(RunLength.encode(input)),sum(input));
    }
}
